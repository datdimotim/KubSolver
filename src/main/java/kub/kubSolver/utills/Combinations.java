...
public final class Combinations{
    public static int schetOrientation(int[] m,int max){
        int x = 0;
        for (int i = m.length-1; i > 0; i--)x +=m[i]*pow(max,(m.length-1-i));
        return x;
    }
    public static int[] schetOrientation(int ch,int max,int length){
    ...
    public static int schetPerestanovka(int[] m){
        int ch = 0;
        int n = m.length;
    ...
    public static int[] schetPerestanovka(int ch,int length){
        int[] m = new int[length];
    ...
    public static int pow(int osn,int stepen){
        int ret=1;
    ...
    public static void perestPair(int[] m, int i, int j) {
        int tmp = m[i];
        m[i] = m[j];
        m[j] = tmp;
    }
